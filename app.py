...
def solve_puzzle():
        ...
        try:
            ...
            if solution:
                ...
                final_result = {
                    'type': 'final_result',
                    'success': True,
                    'final_moves': len(solution) - 1,
                    'total_explored': getattr(solver, 'total_explored', None)
                }
    ...
    return jsonify({
        'success': True,
        'message': '解探索を開始しました。進捗は別途確認してください。',
...
def stop_puzzle():
    ...
    if task_id and task_id in stop_flags:
        ...
        return jsonify({'success': True, 'message': '探索中止要求を送信しました'})
    else:
        return jsonify({'success': False, 'error': '進行中の探索が見つかりません'})
...
def get_progress():
        ...
        try:
            while True:
                try:
                    ...
                    # 進捗データの種類に応じて処理
                    if 'solved' in progress_data:
                        # 最終結果
                        if progress_data['solved']:
                            event_data = {
                                'type': 'final_result',
                                'success': True,
                                'total_explored': progress_data['total_explored'],
                                'final_moves': progress_data['final_moves']
                            }
                        else:
                            event_data = {
                                'type': 'final_result', 
                        ...
                        event_data = {
                            'type': 'progress',
                            'explored_count': progress_data['explored_count'],
                            'current_moves': progress_data['current_moves'],
                    ...
                    # タイムアウト時はハートビートを送信（セッションが有効な場合のみ）
                    if session_id in progress_queues:
    ...
    response = Response(event_stream(session_id), mimetype='text/event-stream')
    response.headers['Cache-Control'] = 'no-cache'
    response.headers['Connection'] = 'keep-alive'
    return response
